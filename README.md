...
| [`championship-league.html`](championship-league.html) | Snooker \u2192 **Championship League** sub-page (All snooker \| Championship League tabs on both pages): scored by the **CHAMPIONSHIP LEAGUE SNOOKER PREDICTION MASTER PROMPT v1.0** ([prompt](docs/CLS_SNOOKER_MASTER_PROMPT.md) \u00b7 [sources](docs/CLS_SNOOKER_SOURCES.md) \u00b7 [irregularities](docs/CLS_SNOOKER_IRREGULARITIES.md)) as an event overlay \u2014 every other snooker event keeps the generic v3.0 prompt. The competition breaks two assumptions the generic prompt makes: the ranking edition is best of four frames, so **2-2 is a real result and the match market is three-way**, and two different tournaments share one name, so the edition (ranking or invitational) is a hard gate that the engine refuses to guess. Three markets per card \u2014 MATCH RESULT, CORRECT SCORE, GROUP WINNER \u2014 with a fifteen-point clearance rule that prints \u201ctoo open to call\u201d rather than naming a winner the data cannot separate. Scoreboard, month calendar and all 42 group tables. The tape is verified rather than trusted: `scripts/build_cls_snooker.mjs` recomputes all 42 group tables from the scorelines and fails the build on any mismatch (253 matches, 168 rows, 0 problems) \u2014 which is how the expunged Kyren Wilson result was caught (IR-CLS-03). Neither prices nor seed numbers are publicly available, so 35 of the 100 match-result points are recorded missing and most match-result reads honestly resolve to SKIP; no return on investment is ever published. |
...
(`docs/SCOTTISH_OPEN_MASTER_PROMPT.md`) on the `scottish-open.html` sub-page.
Snooker follows the same overlay pattern: every event is scored by the generic
SNOOKER PREDICTION MASTER PROMPT v3.0 on `snooker.html`, while Championship
League Snooker — which allows drawn matches and exists in two differently
formatted editions — is scored by the CHAMPIONSHIP LEAGUE SNOOKER PREDICTION
MASTER PROMPT v1.0 (`docs/CLS_SNOOKER_MASTER_PROMPT.md`) on the
`championship-league.html` sub-page. Adding the overlay removed no coverage. Greyhounds likewise run on their
