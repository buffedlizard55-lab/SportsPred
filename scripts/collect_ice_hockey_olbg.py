#!/usr/bin/env python3
"""
Collect the current OLBG Ice Hockey slate into data/ice_hockey_slate.json.

Stdlib only, so it runs on a bare GitHub Actions runner:

    python3 scripts/collect_ice_hockey_olbg.py                 # fetch live, write slate
    python3 scripts/collect_ice_hockey_olbg.py --dry-run       # fetch live, print summary
    python3 scripts/collect_ice_hockey_olbg.py --from FILE     # parse a saved HTML capture
    python3 scripts/collect_ice_hockey_olbg.py --save-html DIR # also dump the raw HTML
...
from lib.ice_hockey_olbg_parse import INDEX_URL, SPORT_ID, parse_index, parse_event_page_markets  # noqa: E402
from lib.olbg_page_health import diagnose  # noqa: E402

ROOT = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
OUT = os.path.join(ROOT, "data", "ice_hockey_slate.json")
...
def build_slate(html: str, source_url: str, enrich: bool = False) -> dict:
    ...
    health = diagnose(html, event_count=len(parsed["events"]), sport="Ice Hockey")
    ...
    return {
        ...
        "source": {
            "name": "OLBG Ice Hockey betting-tips index",
            "url": source_url,
        ...
        "page_health": health,
        "note": (
            "Display-and-join context for the ice hockey engine. OLBG publishes tipster "
            "consensus, not odds, so this slate can never supply a price; prices come from "
            "the ESPN scoreboard odds block (one book, attributed) and the NHL API is used "
            "for fixtures, standings and goaltending."
        ),
    }
...
def main() -> int:
    ...
    if args.save_html:
        ...
        path = os.path.join(args.save_html, f"olbg_ice_hockey_{stamp}.html")
    ...
    for ev in slate["events"]:
        print(f"  {ev['resolved_date'] or ev['display_date']:>16}  {ev['home']} vs {ev['away']}  [{ev['league']}]")
