...
export function renderReport(m) {
  ...
  if (!m.settled) {
    ...
    lines.push('');
    lines.push('Nothing has been recorded or settled yet. Both are automated:');
    lines.push('  1. Recorded predictions — scripts/collect_espn.mjs scores the live ESPN');
    lines.push('     card and appends every selection, append-only.');
    lines.push('  2. Settled results — the same script re-reads finished matches from ESPN');
    lines.push('     and writes the real outcome into data/results.json.');
    lines.push('');
    lines.push('Run: node scripts/collect_espn.mjs   (needs egress to site.api.espn.com)');
    lines.push('');
    lines.push('Note: the games-handicap market stays void regardless, because grading a');
    lines.push('handicap needs the line that was actually offered and no free odds source');
    lines.push('is available (IR-01). A line is never assumed.');
    lines.push('');
