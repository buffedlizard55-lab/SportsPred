#!/usr/bin/env python3
"""
Collect the openly-available OLBG darts markets slate.

Fetches the public OLBG darts tips index (https://www.olbg.com/betting-tips/Darts/15),
parses each event with the pure parser in scripts/lib/darts_olbg_parse.py and
writes data/darts_slate.json. With --enrich it also visits each event page to
record the market groups the server html carries.
...
from scripts.lib.darts_olbg_parse import parse_index, parse_event_page_markets, parse_player_names, INDEX_URL  # noqa: E402
from scripts.lib.olbg_page_health import diagnose  # noqa: E402

OUT_PATH = os.path.join(
    os.path.dirname(os.path.dirname(os.path.abspath(__file__))),
    "data", "darts_slate.json",
)
...
def build_snapshot(html: str, fetched_at: datetime, enrich: bool = False) -> dict:
    ...
    for row in rows:
        ...
        if row.get("type") != "outright" and row.get("matchup"):
            names = parse_player_names(f"<h1>{row['matchup']}</h1>")
            if len(names) == 2:
                player_a = {"name": names[0]}
                player_b = {"name": names[1]}
        events.append({
    ...
    if enrich:
        for ev in events:
            try:
                ...
                if not ev.get("playerA"):
                    names = parse_player_names(page)
                    if len(names) == 2:
                        ev["playerA"] = {"name": names[0]}
                        ev["playerB"] = {"name": names[1]}
    ...
    _health = diagnose(html, event_count=len(events), sport="Darts")
    ...
    return {
        ...
        "notes": [
            "OLBG tipster consensus counts are display-only and are never fed into darts scoring.",
            "OLBG does not expose structured odds in server-rendered HTML, so no price fields exist (IR-DARTS-01).",
            "Tournament, round, venue and date for scored matches are joined from the committed results tape, never from OLBG alone.",
        ],
...
def main() -> int:
    ...
    print(f"Parsed {len(doc['events'])} darts markets from OLBG")
    if args.dry_run:
